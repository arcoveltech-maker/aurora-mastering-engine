...
# ---------------------------------------------------------------------------
# Compatibility aliases and additional helpers
# ---------------------------------------------------------------------------

# Alias for code that uses get_subscription_by_user_id
async def get_subscription_by_user_id(db: AsyncSession, user_id: str) -> Subscription | None:
    return await get_subscription(db, user_id=user_id)


async def update_subscription(db: AsyncSession, sub_id: str, **fields) -> Subscription | None:
    """Update subscription by its own ID (not user_id)."""
...
async def get_user_by_stripe_customer_id(db: AsyncSession, customer_id: str) -> User | None:
    """Find user by looking up stripe_customer_id on their subscription."""
    result = await db.execute(
...
async def get_user_by_email_token(db: AsyncSession, token: str) -> User | None:
    """Stub: in production this would look up an email verification token table."""
    # For now, return None (verification tokens need a separate table/implementation)
    return None


async def reset_track_usage(db: AsyncSession, user_id: str) -> None:
    """Reset track usage for a user (positional-friendly alias)."""
    sub = await get_subscription(db, user_id=user_id)
    if sub:
...
async def increment_track_usage(db: AsyncSession, user_id: str, delta: int = 1) -> None:
    """Increment track usage for a user (positional-friendly alias)."""
    sub = await get_subscription(db, user_id=user_id)
    if sub:
        sub.tracks_used_this_period = (sub.tracks_used_this_period or 0) + delta
        await db.flush()
